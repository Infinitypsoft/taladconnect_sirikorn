        ...
        <div class="container">
            <div class="d-flex justify-space-between align-items-center mt-4 mb-4">
                <v-card-text class="p-0">
                    <h1 class="font-20 m-0">เพิ่มข้อมูลผู้ใช้งาน</h1>
                </v-card-text>
            ...
            <v-card class="mb-6" elevation="2">
                ...
                <v-form ref="form_user_save" v-on:submit.prevent="userSave()">
                        ...
                        <v-row class="container-input-contact">
                            ...
                            <v-col cols="12" xs="12" sm="12" md="4" lg="4" xl="4">
                                ...
                                <v-text-field hide-details="auto" type="password" label="กรุณากรอกรหัสผ่าน"
                                    v-model="password" :rules="rules.password" outlined dense single-line
                                    required></v-text-field>
                            </v-col>
                            ...
                            <v-col cols="12" xs="12" sm="12" md="4" lg="4" xl="4">
                                ...
                                <v-text-field hide-details="auto" type="password" label="กรุณากรอกยืนยันรหัสผ่าน"
                                    v-model="confirm_password" :rules="rules.confirm_password" outlined dense
...
export default {
    data() {
        return {
            ...
            rules: {
                ...
                password: [val => (val || '').length > 0 || 'ต้องระบุข้อมูลในช่องนี้'],
                confirm_password: [val => (val || '').length > 0 || 'ต้องระบุข้อมูลในช่องนี้'],
            },
    ...
    methods: {
        async userSave() {
            if (this.$refs.form_user_save.validate()) {
                this.loading_page = true;
                if (this.password !== this.confirm_password) {
                    this.loading_page = false;
                    this.alertData('error', 'ทำรายการไม่สำเร็จ', 'รหัสผ่านไม่ตรงกัน');
                    return;
                }
                let config = {
                    headers: {
                ...
                await admin.adminAdd(data, config).then((result) => {
