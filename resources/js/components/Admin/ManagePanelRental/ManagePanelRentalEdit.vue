                                ...
                                <v-form ref="form_panelrental_add" v-on:submit.prevent="persist">
                                    ...
                                    <v-card-text>
                                        ...
                                        <v-row class="text-center">
                                            <v-col cols="12">
                                                <v-btn :disabled="is_block_empty" type="submit" outlined
...
export default {
    ...
    methods: {
        async checkSelectBlockList(){
            let check = this.select_block_list.filter(v=>v.is_delete === 0)
            if(check.length === 0 ){
                this.is_block_empty = true
            }else{
                this.is_block_empty = false
            }
        },
        ...
        async getActiveColor() {
            ...
            this.siteData.block_list.map(data => {
                ...
                let check = this.get_data.block_list.filter(v => v.block_name === data.block_name)
                ...
                resultArray.map((cellKey, i) => {
                    if (this.$refs[cellKey]) {
                        ...
                        // บล็อกถูกเลือกไปแล้ว
                        if ((data.tennant_member_id !== null && check.length === 0) || data.status === 3) {
        ...
        persist() {
            if (this.$refs.form_panelrental_add.validate()) {
                // console.log(this.select_block_list)
                const setAddObject = {
                ...
                window.localStorage.setItem("managepanelrental_edit", JSON.stringify(setAddObject));
                window.localStorage.setItem("managepanelrental_edit_select_block_list", JSON.stringify(this.select_block_list));
                this.$router.push(`/managepanelrental/editsteptwo?id=${this.$route.query.id}`)
            }
        },
    },
    created() {
