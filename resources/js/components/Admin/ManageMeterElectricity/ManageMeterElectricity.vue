    ...
    <div class="container">
      <div class="d-flex justify-space-between align-items-center mt-4 mb-4">
        <v-card-text class="p-0">
          <h1 class="font-20 m-0">จัดการมิเตอร์ไฟ</h1>
        </v-card-text>
      ...
      <!-- เพิ่มมิเตอร์ไฟ -->
      <v-dialog v-model="dialog_meter_electricity" width="1054">
        <v-card>
          <v-card-title
          ...
          >
            ...
            <v-btn
              @click="
                () => {
                  dialog_meter_electricity = false;
                }
              "
              class="p-0 m-0"
              width="50"
          ...
          <v-card class="mt-2 p-5 pb-3 pt-3" elevation="0">
            <v-form ref="form_electric_add" v-on:submit.prevent="update()">
                  ...
                  >
                    <v-card-text class="p-0 mt-2">
                      <div class="d-flex align-center">
                        <img
                          class="mr-2"
                          :src="base_url + `/images/icon/electricity.png`"
                          alt="electricity"
                          width="20"
                          height="20"
                          dark
                        ...
                        <span class="font-16 color-title-card"
                          >ข้อมูลมิเตอร์ไฟปัจจุบันของแผง {{block_name}}
...
import meter_electric from "../../../api/meter_electric.js";
export default {
  data() {
    return {
      dialog_detail: false,
      ...
      await meter_electric
        ...
        .then((result) => {
          if (result.data.success === true) {
            ...
            setTimeout(() => {this.alert_success = false, this.dialog_meter_electricity = false}, 2000)
            this.loading_page = false;
          }
        })
        .catch((error) => {
