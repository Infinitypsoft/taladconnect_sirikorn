                            ...
                            <v-col xs="12" sm="12" md="12" lg="8" xl="8">
                                ...
                                <div class="container-image-upload">
                                    <div class="slide-image-upload">
                                        <!-- <img :src="base_url + `/images/picture.png`" width="140" height="140"
                                            alt="Picture" /> -->
                                ...
                                <v-form ref="form_banner_add" v-on:submit.prevent="bannerUpdate()">
                                    ...
                                    <v-card-text class="p-0 mt-8">
                                        ...
                                        <div>
                                            ...
                                            <!-- <v-file-input outlined dense single-line label="กรุณาเลือกไฟล์รูปภาพ"
                                    ...
                                    <v-card-text class="p-0">
                                        ...
                                        <div>
                                            ...
                                            <v-text-field label="กรุณากรอกคำเชิญชวนที่จะแสดงหน้าเว็บไซต์ เป็นภาษาไทย"
                                                v-model="data_banner.title_thai" outlined dense single-line
                                                :rules="invitation_thRules" :counter="27"></v-text-field>
                                        </div>
                                        <div>
                                            <p class="font-14 text-ex">ภาษาอังกฤษ</p>
                                            <v-text-field :rules="invitation_enRules" :counter="25"
                                                v-model="data_banner.title_eng" outlined dense single-line
                                    ...
                                    <v-card-text class="p-0">
                                        ...
                                        <div>
                                            <p class="font-14 mt-3 text-ex">ภาษาไทย</p>
                                            <v-textarea outlined single-line v-model="data_banner.description_thai"
                                                label="กรุณากรอกรายละเอียดที่จะแสดงหน้าเว็บไซต์ เป็นภาษาไทย"
                                                :rules="detail_thRules" :counter="120"></v-textarea>
                                        </div>
                                        <div>
                                            <p class="font-14 text-ex">ภาษาอังกฤษ</p>
                                            <v-textarea outlined single-line v-model="data_banner.description_eng"
...
export default {
    ...
    methods: {
        async getBanner() {
            ...
            await banner.bannerDetail(this.base64Decode(this.$route.query.id), config).then((result) => {
                if (result.data.success === true) {
                    this.data_banner = result.data.data

                    this.content_picture = { name: this.data_banner.pic }
                    this.imagePreviewURL = this.base_url + `/public/` + this.data_banner.pic_path
        ...
        async bannerUpdate() {
            if (this.$refs.form_banner_add.validate()) {
                ...
                data.append('id', this.data_banner.id)
                
                if(this.content_picture.size !== undefined){
                    data.append('pic', this.content_picture);
                }
                data.append('title_thai', this.data_banner.title_thai)
                data.append('title_eng', this.data_banner.title_eng)
                data.append('description_thai', this.data_banner.description_thai)
                data.append('description_eng', this.data_banner.description_eng)

                await banner.bannerUpdate(data, config).then((result) => {
                    if (result.data.success === true) {
                        this.loading_page = false;
